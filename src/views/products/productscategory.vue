      ...
      <el-header class="h-auto">
        <el-row class="flex-row flex-between">
          <div class="flex-row">
            ...
            <el-button class="ml-2" :icon="Search" circle @click="getProductsCategoryListFun" />
          </div>
          <div>
...
const submitForm = async() => {
  ...
  if ( flag ) {
    if ( form.id ) {
      // 编辑
      const res = await updateProductsCategory( form )
      if ( res.data ) {
        ElMessage( {
          message : res.msg,
          type : 'success'
      ...
      // 添加
      const res = await addProductsCategory( form )
      if ( res.data ) {
        ElMessage( {
          message : res.msg,
          type : 'success'
...
const edit = async row => {
  const res = await getProductsCategoryById( { id : row.id } )
...
const dels = row => {
  ...
  ElMessageBox.confirm( `确定要删除这些选中分类吗吗`, {
    ...
    const res = await deleteProductsCategory( { ids : arr } )
    if ( res.code == 200 ) {
      ElMessage( {
        type : 'success',
        message : `删除成功`
...
const changgestateFun = async row => {
  const { id } = row
  const res = await enableProductsCategory( { id } )
  if ( res.data ) {
    ElMessage( {
      type : 'success',
      message : res.msg
